...
public class YoloDetector
{
    ...
    private static readonly string[] TargetIds =  {"a1", "a2", "a3", "c1", "c2", "c3"};
    ...
    private void InitDetectSession(string modelPath, bool useCuda = false)
    {
        ...
        if (useCuda)
        {
            ...
            catch (Exception e)
            {
                ...
                Console.WriteLine("Failed to use CUDA, fallback to CPU.");
            }
        }
    ...
    public IEnumerable<float[]> InferenceResult(in Tensor<float> tensor)
    {
    ...
    public IEnumerable<YoloResult> PostProcess(IEnumerable<float[]> scores, float ratio)
    {
        ...
        foreach (var score in scores)
        {
            ...
            boxes.Add(ToRect(score.AsSpan()[..4], ratio));
        }
    ...
    public IEnumerable<YoloResult> Detect(in Tensor<float> tensor, float ratio = 2240 / 640f)
    {
    ...
    public void Warmup() => InferenceResult(new DenseTensor<float>(new [] {1, 3, 640, 640}));
    public float GetRatio(Size size) => Math.Max(size.Width, size.Height) / (float)ModelSize;
}